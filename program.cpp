...
void check_cmd(const char* input, int* cmd);
...
void check_cmd(const char* input,int* cmd){
	const char* commands[4] = { "about", "help", "info", "unpack" };
	for(int i=0; i<4; i++){
		if(memcmp(input, (char*)commands[i], strlen(commands[i])) == 0)
			*cmd = i;
	}
}
